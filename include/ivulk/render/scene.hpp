...
namespace ivulk {
	...
	public:
		...
		{
			static_assert(std::is_base_of_v<I_Renderable, R>, "Template parameter `R` must derive from interface `I_Renderable`");
		...
		virtual inline int16_t renderOrder() const override
		{
			return E_RenderPriority::Normal - 100;
		}
	...
	private:
		...
		{
			inline bool operator()(const std::shared_ptr<I_Renderable>& a, const std::shared_ptr<I_Renderable>& b) const
			{
