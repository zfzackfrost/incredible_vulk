...
namespace ivulk {
	...
	VkShaderModule GraphicsPipeline::createShaderModule(VkDevice device, const std::vector<char>& shaderCode,
	...
	GraphicsPipeline* GraphicsPipeline::createImpl(VkDevice device, GraphicsPipelineInfo info)
	{
		...
		if (info.shaderPath.frag.has_value())
			shaderModules.push_back(
				pathToShaderModule(*info.shaderPath.frag, VK_SHADER_STAGE_FRAGMENT_BIT));
		...
		if (info.shaderPath.geom.has_value())
			shaderModules.push_back(
				pathToShaderModule(*info.shaderPath.geom, VK_SHADER_STAGE_GEOMETRY_BIT));
