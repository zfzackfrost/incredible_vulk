...
namespace ivulk {
    ...
    VkExtent2D App::chooseVkSwapExtent(const VkSurfaceCapabilitiesKHR& capabilties)
    {
        ...
        else
        {
            ...
            actualExtent.width = clamp_ui(
                actualExtent.width, capabilties.minImageExtent.width, capabilties.maxImageExtent.width);
            actualExtent.height = clamp_ui(
                actualExtent.height, capabilties.minImageExtent.height, capabilties.maxImageExtent.height);
            return actualExtent;
        }
    }
    ...
    SwapChainInfo App::querySwapChainInfo(VkPhysicalDevice device)
    {
        ...
        if (formatCount != 0)
        {
            ...
            vkGetPhysicalDeviceSurfaceFormatsKHR(
                device, state.vk.surface, &formatCount, scInfo.formats.data());
        }
        ...
        if (presentModesCount != 0)
        {
            ...
            vkGetPhysicalDeviceSurfacePresentModesKHR(
                device, state.vk.surface, &presentModesCount, scInfo.presentModes.data());
        }
    ...
    void App::createVkSwapChain()
    {
        ...
        vkGetSwapchainImagesKHR(
            state.vk.device, state.vk.swapChain.sc, &imageCount, state.vk.swapChain.images.data());
